...
export class ResourcesUtil {
  ...
  static buildResourcesMap(absolutePath: string) {
    this.resourcesMap[this.getFileName(absolutePath)] = this.originDataByGlob[absolutePath].default
  }
  ...
  static loadAllResources() {
    if (this.isStoreEmpty()) {
      ...
      Object.keys(this.originDataByGlob).forEach(this.buildResourcesMap.bind(this))
  ...
  static getSrc(resourcesName: string) {
    ...
    resourcesName = resourcesName.includes('.png') ? resourcesName : resourcesName.concat('.png')
